...
function MainPage(props) {
  const mainItemImageRef = useRef(null);
  const mainPartImageRef = useRef(null);
  const mainStylemageRef = useRef(null);
  ...
  useEffect(() => {
    ...
    observer.observe(mainItemImageRef.current);
    observer.observe(mainPartImageRef.current);
    observer.observe(mainStylemageRef.current);
    return () => {
      observer.disconnect();
    };
  }, []);
